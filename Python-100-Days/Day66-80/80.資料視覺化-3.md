...
如果想在 JupyterLab 中使用 pyecharts 繪圖，我們還需要做一些準備工作，主要是修改 pyecharts 的配置，程式碼如下所示。
...
在執行完上面的程式碼後，我們就可以透過呼叫`bar`物件的方法來完成對圖表的渲染。如果直接使用`render`方法，那麼繪製好的統計圖表將儲存到一個 HTML 檔案中，開啟該檔案也能夠看到繪製好的統計圖表，而`render_notebook`方法則是將圖表渲染到瀏覽器視窗中。

```python
bar_chart.render_notebook()
```

上面程式碼的執行效果如下圖所示。值得一提的是，下圖中的標題、圖例、右側的工具箱都是可以點選的，大家可以點選它們看看會有什麼樣的效果，ECharts 的魅力就在於它的互動效果，大家一定要試一試。
