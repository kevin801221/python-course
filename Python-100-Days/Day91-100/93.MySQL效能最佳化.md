...
1. 讓MySQL迴歸儲存的基本職能：MySQL 資料庫只用於資料的儲存，不進行資料的複雜計算，不承載業務邏輯，確儲存儲和計算分離；
...
5. 調整`innodb_buffer_pool_size`：InnoDB資料和索引的記憶體緩衝區大小，以位元組為單位，這個值設定得越高，訪問表資料需要進行的磁碟I/O操作就越少，如果可能甚至可以將該值設定為實體記憶體大小的80%。
