...
下面我們以接入[七牛雲](https://www.qiniu.com/)為例，講解如何實現將使用者上傳的檔案儲存到七牛雲端儲存。七牛雲是國內知名的雲端計算及資料服務提供商，七牛雲在海量檔案儲存、CDN、影片點播、互動直播以及大規模異構資料的智慧分析與處理等領域都有自己的產品，而且非付費使用者也可以免費接入，使用其提供的服務。下面是接入七牛雲的流程：
