...
接下來，我們先看看如何透過Django的配置檔案來配置日誌。Django的日誌配置基本可以參照官方文件再結合專案實際需求來進行，這些內容基本上可以從官方文件上複製下來，然後進行區域性的調整即可，下面給出一些參考配置。
...
可見，Django的ORM框架允許我們用物件導向的方式完成關聯式資料庫中的分組和聚合查詢。
