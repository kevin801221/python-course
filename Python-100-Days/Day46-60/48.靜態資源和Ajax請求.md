...
配置好靜態資源之後，大家可以執行專案，然後看看之前我們寫的頁面上的圖片是否能夠正常載入出來。需要說明的是，在專案正式部署到線上環境後，我們通常會把靜態資源交給專門的靜態資源伺服器（如Nginx、Apache）來處理，而不是有執行Python程式碼的伺服器來管理靜態資源，所以上面的配置並不適用於生產環境，僅供專案開發階段測試使用。使用靜態資源的正確姿勢我們會在後續的章節為大家講解。
